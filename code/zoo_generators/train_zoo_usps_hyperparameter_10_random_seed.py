...
def main():
    ...
    # experiment name
    project = "Corr: USPS Random Seed"
    experiment_name = "tune_zoo_usps_hyperparameter_10_random_seed"
    ...
    net_dir = PATH_ROOT.joinpath("zoos/USPS")
    try:
    ...
    usps_path = PATH_ROOT.joinpath("data/USPS")
    print(f"Data directory: {usps_path.absolute()}")
    try:
        # load existing dataset
        dataset = torch.load(str(usps_path.joinpath("dataset.pt")))
        ...
        transform = transforms.Compose(
            [
                # resize to expected size of small cnn
                transforms.Resize((28, 28)),
                transforms.ToTensor(),
                transforms.Normalize((0.1307,), (0.3081,)),
            ])

        val_and_trainset_raw = datasets.USPS(
            usps_path, train=True, download=True, transform=transform)
        testset_raw = datasets.USPS(
            usps_path, train=False, download=True, transform=transform)
        trainset_raw, valset_raw = torch.utils.data.random_split(
            val_and_trainset_raw, [6000, 1291], generator=torch.Generator().manual_seed(dataset_seed))
        ...
        torch.save(dataset, usps_path.joinpath("dataset.pt"))
