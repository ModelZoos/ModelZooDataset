...
def main():
    ...
    # experiment name
    project = "SVHN Hyperparameter Random Seed"
    experiment_name = "tune_zoo_svhn_hyperparameter_10_random_seeds"
    ...
    config["model::dropout"] = tune.grid_search([0.0, 0.3, 0.5])
    config["model::init_type"] = tune.grid_search(
        ["uniform", "normal", "kaiming_uniform", "kaiming_normal"])
    config["optim::optimizer"] = "sgd"
    config["optim::lr"] = tune.grid_search([1e-2, 1e-3, 1e-4])
    config["optim::wd"] = tune.grid_search([0, 1e-3, 1e-1])
    ...
    net_dir = PATH_ROOT.joinpath("zoos/SVHN")
    try:
    ...
    svhn_path = PATH_ROOT.joinpath("data/SVHN")
    print(f"Data directory: {svhn_path.absolute()}")
    try:
        # load existing dataset
        dataset = torch.load(str(svhn_path.joinpath("dataset.pt")))
        ...
        val_and_trainset_raw = datasets.SVHN(
            svhn_path, train=True, download=True, transform=transform)
        testset_raw = datasets.SVHN(
            svhn_path, train=False, download=True, transform=transform)
        trainset_raw, valset_raw = torch.utils.data.random_split(
            val_and_trainset_raw, [60000, 13257], generator=torch.Generator().manual_seed(dataset_seed))
        ...
        torch.save(dataset, svhn_path.joinpath("dataset.pt"))
    ...
    analysis = tune.run(
        ...
        resume=False,  # does not resume from previous run. if run should be done all over, set resume=False
        # resume=True, # resumes local runs, same as resume="LOCAL"
